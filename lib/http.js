...
// FIXME: Move this to the configuration part of the application. This was done
...
const maxRequestsPerSecond = parseInt(process.env.RATE_LIMIT || 1000, 10);

const MAX_CACHE_SIZE = 60 * maxRequestsPerSecond; // max request per minute
const MAX_CACHE_AGE = 60 * 1000; // 1 minute

const CACHE = new LRU({max: MAX_CACHE_SIZE, maxAge: MAX_CACHE_AGE});
config['rate-limit'].cache = CACHE;
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  const agent = new https.Agent({
    rejectUnauthorized: config['https-agent'].rejectUnauthorized,
    keepAlive: true,
  ...
  app.post(
    routes.rsvps,
    rateLimiter({monitor: true}),
    validate('bedrock-rsvp.postRsvpRequest'),
    asyncHandler(async (req, res) => {
      const result = await api.createRequest(req.body);
      const rsvpId = result.rsvp.id;

      handleTimeout({req, rsvpId});

      const response = {
        created: result.meta.created,
        ttl: result.rsvp.ttl,
        url: `${baseRsvpUrl}/${rsvpId}`,
      };
      res.json(response);
    }));
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      req.on('error', error => {
        logger.error('The RSVP listener connection has failed.', {
          rsvpId,
          error,
        });
      });
...
function rateLimiter({monitor = true}) {
  const middlewares = [];
  if(config['rate-limit'].enable) {
    middlewares.push(limiter);
  }
  if(monitor) {
    middlewares.push(responseTimer);
  }
  return middlewares;
}

function handleTimeout({req, rsvpId}) {
  req.on('timeout', () => {
    logger.info('The RSVP connection has timed out.', {
      rsvpId
    });
    req.destroy();
  });
